...
function getBookingErrorMessage(error) {
  ...
  if (normalizedMessage.includes('conflict')) {
    return 'That resource is already booked for the selected date and time.'
  }

  if (normalizedMessage.includes('out of service')) {
    return 'The selected resource is currently out of service and cannot be booked.'
  }

  if (normalizedMessage.includes('capacity')) {
    return 'Expected attendees exceed the selected resource capacity.'
  }

  if (normalizedMessage.includes('past')) {
  ...
  return error.message || 'Unable to complete the booking action right now.'
}

function getBookingFieldErrors(error) {
...
export default function BookingsPage() {
  ...
  const load = async () => {
    try {
      const [resourceResponse, bookingResponse, notificationResponse] = await Promise.all([
        api.get('/resources'),
        api.get('/bookings'),
      ...
      const nextBookings = [...bookingResponse.data.data].sort((left, right) => {
  ...
  const createBooking = async (e) => {
    ...
    try {
      ...
      setSubmitSuccess(response.data.message || 'Booking request submitted successfully.')
      setForm(createInitialForm())
      await load()
    } catch (error) {
      setFieldErrors(getBookingFieldErrors(error))
      setSubmitError(getBookingErrorMessage(error))
    } finally {
  ...
  const runBookingAction = async (booking, action) => {
    ...
    try {
      if (action === 'APPROVED' || action === 'REJECTED') {
        const response = await api.patch(`/bookings/${booking.id}/status`, {
          status: action,
          reason: action === 'APPROVED' ? 'Approved by admin' : 'Rejected by admin'
        })

        setSubmitSuccess(
          response.data.message || `Booking ${action === 'APPROVED' ? 'approved' : 'rejected'} successfully.`
        )
      }

      if (action === 'CANCELLED') {
        const response = await api.patch(`/bookings/${booking.id}/cancel`)
        setSubmitSuccess(response.data.message || 'Booking cancelled successfully.')
      }
