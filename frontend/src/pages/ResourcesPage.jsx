...
function getErrorMessage(error) {
  ...
  if (error.response?.status === 403) {
    return 'Only admins can manage resources.'
  }
  ...
  return error.message || 'Unable to save the resource right now.'
}
...
export default function ResourcesPage() {
  ...
  const load = async () => {
    console.log('[resources] load started')
    ...
    try {
      ...
      console.log('[resources] load succeeded', {
        resourceCount: resourceResponse.data.data.length,
        notificationCount: notificationResponse.data.data.length
      })
  ...
  const saveResource = async (e) => {
    ...
    console.log('[resources] submit handler fired', {
    ...
    try {
      ...
      console.log(`[resources] ${isEditing ? 'update' : 'create'} succeeded`, response.data)

      resetFormState()
      setSubmitSuccess(
        response.data.message || (isEditing ? 'Resource updated successfully.' : 'Resource created successfully.')
      )
      await load()
    } catch (error) {
      console.error(`[resources] ${isEditing ? 'update' : 'create'} failed`, error)
  ...
  const deleteResource = async (resource) => {
    ...
    console.log('[resources] delete requested', { id: resource.id, name: resource.name })
    ...
    try {
      ...
      console.log('[resources] delete succeeded', { id: resource.id })
