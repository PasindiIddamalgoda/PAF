...
function getTicketErrorMessage(error) {
  ...
  if (normalizedMessage.includes('valid ticket category')) {
    return 'Select one of the supported maintenance categories.'
  }

  if (normalizedMessage.includes('campus location')) {
    return 'Add a location or select a related resource before submitting the ticket.'
  }

  if (normalizedMessage.includes('preferred contact')) {
    return 'Add an email address, phone number, or extension so staff can contact you.'
  }

  if (normalizedMessage.includes('rejection reason')) {
    return 'Add a rejection reason before rejecting the ticket.'
  }

  if (normalizedMessage.includes('assigned staff') || normalizedMessage.includes('assigned user')) {
    return 'Assign the ticket to a technician or support staff member first.'
  }
  ...
  if (error.response?.status === 403) {
    return 'You do not have permission to perform that ticket action.'
  }

  if (error.response?.status === 401) {
    return 'Your session is no longer authorized. Please log in again.'
  }

  return error.message || 'Unable to complete the ticket action right now.'
}

function getTicketFieldErrors(error) {
...
export default function TicketsPage() {
  ...
  const load = async () => {
    try {
      const [resourceResponse, ticketResponse, notificationResponse] = await Promise.all([
        api.get('/resources'),
        api.get('/tickets'),
        api.get('/notifications')
      ])
      const nextResources = [...resourceResponse.data.data].sort((left, right) => left.name.localeCompare(right.name))
      const nextTickets = [...ticketResponse.data.data].sort((left, right) => {
        const leftValue = left.createdAt ? new Date(left.createdAt).getTime() : Number(left.id || 0)
        const rightValue = right.createdAt ? new Date(right.createdAt).getTime() : Number(right.id || 0)
      ...
      const commentResponses = await Promise.all(
        nextTickets.map((ticket) => api.get(`/tickets/${ticket.id}/comments`))
      )

      const nextComments = {}
      nextTickets.forEach((ticket, index) => {
        nextComments[ticket.id] = commentResponses[index].data.data
      })
      setComments(nextComments)
    } catch (error) {
      setSubmitError(getTicketErrorMessage(error))
    }
  }
  ...
  const createTicket = async (e) => {
    ...
    try {
      ...
      setSubmitSuccess(response.data.message || 'Ticket created successfully.')
      setForm(initialForm)
      await load()
    } catch (error) {
      setFieldErrors(getTicketFieldErrors(error))
      setSubmitError(getTicketErrorMessage(error))
    } finally {
  ...
  const runTicketStatusAction = async (ticket, nextStatus) => {
    ...
    try {
      const response = await api.patch(`/tickets/${ticket.id}/status`, {
        status: nextStatus,
        rejectionReason: nextStatus === 'REJECTED' ? rejectionReason : '',
        resolutionNotes:
          nextStatus === 'RESOLVED' || nextStatus === 'IN_PROGRESS' || nextStatus === 'CLOSED' ? resolutionNotes : ''
      })

      setSubmitSuccess(response.data.message || `Ticket marked as ${toTitleCase(nextStatus)}.`)
      setTicketDrafts((currentDrafts) => ({
        ...currentDrafts,
        [ticket.id]: {
  ...
  const assignTicket = async (ticket) => {
    ...
    try {
      const response = await api.patch(`/tickets/${ticket.id}/assign`, {
        technicianId: Number(technicianId)
      })

      setSubmitSuccess(response.data.message || 'Ticket assigned successfully.')
      await load()
    } catch (error) {
      setSubmitError(getTicketErrorMessage(error))
    } finally {
  ...
  const addComment = async (ticket) => {
    ...
    try {
      const response = await api.post(`/tickets/${ticket.id}/comments`, { content })
      setSubmitSuccess(response.data.message || 'Comment added successfully.')
      setTicketDrafts((currentDrafts) => ({
        ...currentDrafts,
        [ticket.id]: {
          ...currentDrafts[ticket.id],
