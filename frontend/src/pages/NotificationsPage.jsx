...
function getNotificationErrorMessage(error) {
  ...
  if (typeof message === 'string' && message.toLowerCase().includes('no static resource')) {
    return 'Notification action could not be completed. Please try again.'
  }
...
export default function NotificationsPage() {
  ...
  const load = async ({ preserveMessage = false } = {}) => {
    ...
    try {
      ...
      setNotifications([])
      setPageError(getNotificationErrorMessage(error))
    } finally {
      setIsLoading(false)
  ...
  const markRead = async (notificationId) => {
    ...
    try {
      ...
      setPageSuccess(response.data?.message || 'Notification marked as read.')
  ...
  const markAllRead = async () => {
    ...
    try {
      ...
      setPageError('')
      setPageSuccess(response.data?.message || 'All notifications marked as read.')
    } catch (error) {
      setPageError(getNotificationErrorMessage(error))
    } finally {
      setActiveNotificationId(null)
      ...
      <section className="panel colorful-1 notifications-shell">
        ...
        {isLoading ? (
          <div className="empty-state notifications-empty-state">
            ...
            <p>The latest campus updates are being prepared.</p>
          </div>
